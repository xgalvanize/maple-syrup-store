...
export default function ProductCard({ product, onAdd, isAdding = false }) {
  const price = (product.priceCents / 100).toFixed(2);

  return (
    <article className="card" title="Click Add to Cart to purchase this product">
      {product.imageUrl && (
      ...
      <div className="card-body">
        ...
        <div className="row">
          <strong title="Price per unit">${price}</strong>
          <button
            className="button"
            type="button"
            onClick={() => onAdd(product.id)}
            title="Add this item to your shopping cart"
            disabled={isAdding}
          >
